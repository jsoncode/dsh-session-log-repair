/**
 * dsh-session-repair — host half.
 *
 * Repairs a DSH JSONL session log whose committed region contains a seq
 * collision: two writers appended different events under the same `seq` values
 * (a stalled writer resuming after another process committed a crash repair).
 * The loader refuses such a log, so the session cannot be opened.
 *
 ...
 *
 * Surfaces: model tools (`dsh_session_repair_*`), the `/dsh-session-repair`
 * command, and the fenced HTTP route `/dsh-session-repair/api` used by the
 * browser half.
 */
...
const API_PATH = '/dsh-session-repair/api'
...
/**
 * Resolve a host package the way the host itself does. A plain import works
 * when the plugin carries its own node_modules; otherwise resolution falls back
 * to the DSH profile's node_modules, which is where the running host loaded the
 * same instance from.
...
async function loadHostPackage(specifier) {
  ...
  } catch { /* no profiles directory: the plain import failure below is the report */ }
  bases.push(join(profiles, 'package.json'))
  for (const base of bases) {
    try {
      const resolved = createRequire(base).resolve(specifier)
      return await import(pathToFileURL(resolved).href)
    } catch { /* try the next profile root */ }
  }
  throw new Error(
    `cannot resolve "${specifier}" (host package); plugin import failed: `
...
async function inspectSession(deps, entry) {
  ...
  try {
    ...
    return {
      ...base,
      status: SEQ_GAP.test(message) ? 'corrupt' : 'unreadable',
      repairable: SEQ_GAP.test(message),
      message,
    }
  }
}
...
async function repairOne(deps, entry, request, helpers) {
  ...
  const { header, rows } = parseLog(raw.content, decodeStorageRecord)
  ...
  const result = {
    ...
    survivor: describeRow(plan.survivor),
...
async function runOp(deps, request, helpers) {
  ...
  if (op === 'status') {
    ...
    const toolNames = ['dsh_session_repair_scan', 'dsh_session_repair_apply', 'dsh_session_repair_verify']
    return {
      ok: true,
...
export async function apply(ctx, config) {
  ...
  ctx.inject(['skills'], (skillCtx) => {
    ...
    try {
      ...
      skillCtx.logger.warn(`dsh-session-repair: skill not registered — ${cause instanceof Error ? cause.message : String(cause)}`)
      return
    }
    skillCtx.effect(() => skills.register(skill), 'dsh-session-repair: skill')
  })
  ...
  })), 'dsh-session-repair: tool dsh_session_repair_scan')

  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'dsh_session_repair_apply',
    description: '修复 DSH 会话日志的 seq 冲突：保留与文件末尾相连的存活写入者链，丢弃重叠的旧版本行。'
      + ' Repair a DSH session log seq collision: keep the writer chain that reaches the end of the file and drop the overlapping older version. '
  ...
  })), 'dsh-session-repair: tool dsh_session_repair_apply')

  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'dsh_session_repair_verify',
  ...
  })), 'dsh-session-repair: tool dsh_session_repair_verify')
}

 ...
 *
 * Reading uses the running backend (`ctx.sessionPersistence`) when it still
 * exposes the legacy raw/load surface (dsh < 0.1.5); on the format-aware host
 * (dsh >= 0.1.5, where `@deepseek-ai/dsh-session` dropped `decodeStorageRecord`
 * and the backend switched to a handle-based API) the plugin reads the stored
 * artifact itself (splitting concatenated zstd frames) and decodes each row
 * with a vendored, self-contained storage-record expander so it still works
 * across both legacy `session.jsonl` and generation-named `session.vN.jsonl`
 * artifacts without depending on the removed host symbol. Writing re-frames the
 * repaired plaintext with Node's zstd (frame 1 is exactly the header line, the
 * remainder is one frame), and the result is re-loaded through the backend
...
import {
  closeSync, copyFileSync, fsyncSync, mkdirSync, openSync, readFileSync, readdirSync,
...
const SEQ_GAP = /seq gap(?:\s+in committed region)?/
/**
...
/**
 * Canonical session-log basename: `session.jsonl` (format v0) or the
 * generation-named `session.vN.jsonl`, each optionally `.zstd`-compressed. A
 * session directory may hold several generations (the harness keeps the
 * previous raw log alongside a migrated one), so discovery picks the file the
 * host actually reads — the highest format generation, then zstd over plaintext.
 */
...
/** Zstandard frame magic bytes as a little-endian uint32. */
const ZSTD_FRAME_MAGIC = 0x28b52ffd

/**
 * Locate the byte offset of every complete zstd frame in a buffer. The harness
 * writes a session log as a sequence of concatenated frames (header line is one
 * frame, the remainder is appended in batches), and Node's one-shot
 * `zstdDecompressSync` only consumes the FIRST frame — so a full read must
 * split on the frame magic and decompress each frame independently. This mirrors
 * the harness persistence backend's own frame scan.
 * @param buf - the zstd-compressed bytes (a Buffer).
 * @returns the start offset of every frame in file order.
 */
function zstdFrameStarts(buf) {
  const starts = []
...
/**
 * Decompress a concatenated-frame zstd buffer the way a harness session log on
 * disk is laid out. A single complete frame is left to Node's one-shot decoder;
 * multi-frame input is split on the frame magic and reassembled.
 * @param bytes - the stored zstd bytes (any byte source accepted by Buffer.from).
 * @returns the decompressed plaintext as a Buffer.
 */
function decompressZstdFrames(bytes) {
  const buf = Buffer.from(bytes)
  const starts = zstdFrameStarts(buf)
  if (starts.length === 0) return Buffer.from(zstdDecompressSync(buf))
  const parts = []
  for (let k = 0; k < starts.length; k += 1) {
    const start = starts[k]
    const end = k + 1 < starts.length ? starts[k + 1] : buf.byteLength
    parts.push(Buffer.from(zstdDecompressSync(buf.subarray(start, end))))
  }
  return Buffer.concat(parts)
}

/* ──────────────── vendored storage-record decoder (compat) ──────────────── */
/*
 * The host used to expose `@deepseek-ai/dsh-session.decodeStorageRecord`, which
 * expanded packed `assistant/chunk` runs (rows tagged `text-chunks` /
 * `reasoning-chunks` / `tool-call-chunks`, carrying `seq0`/`time0`) back into
 * the individual chunk events while passing every other storage record through
 * as-is. dsh >= 0.1.5 removed that export, so this plugin vendors an
 * equivalent, self-contained expander. It decodes BOTH legacy logs (standalone
 * chunk rows, which it expands) and the generation-3 layout (where chunk data
 * is folded into `assistant/message` and every line is a single event, which it
 * passes through verbatim). The vendored logic is a faithful port of the old
 * chunk-rows codec (minimum run 3).
 */
...
/** Admit a numeric value as a non-negative safe integer sequence position. */
function admitSessionSeq(value) {
...
function chunkRowMalformed(tag, why) {
  throw new Error(`malformed ${tag} storage row: ${why}`)
}

/** Validate the shared run-data fields; returns the payload array. */
function validateChunkRunData(tag, data, payloadKey) {
  if (typeof data.turn !== 'number' || typeof data.step !== 'number' || typeof data.index !== 'number') {
    chunkRowMalformed(tag, 'turn/step/index must be numbers')
  }
  const payload = data[payloadKey]
  if (!Array.isArray(payload) || payload.length === 0 || payload.some((entry) => typeof entry !== 'string')) {
    chunkRowMalformed(tag, `${payloadKey} must be a non-empty string array`)
  }
  const dt = data.dt
  if (!Array.isArray(dt) || dt.some((gap) => !Number.isSafeInteger(gap))) {
    chunkRowMalformed(tag, 'dt must be an array of safe integers')
  }
  if (dt.length !== payload.length - 1) {
    chunkRowMalformed(tag, `dt length ${dt.length} does not match ${payload.length} members`)
  }
  return payload
}

/** Validate a chunk-row value's envelope and data, throwing on any malformation. */
function validateChunkRow(value, tag) {
  if (!hasExactKeys(value, ['type', 'seq0', 'time0', 'data'])) {
    chunkRowMalformed(tag, 'envelope must be exactly {type, seq0, time0, data}')
  }
  if (!Number.isSafeInteger(value.seq0) || value.seq0 < 0 || Object.is(value.seq0, -0)) {
    chunkRowMalformed(tag, 'seq0 must be a non-negative safe integer')
  }
  if (!Number.isSafeInteger(value.time0)) chunkRowMalformed(tag, 'time0 must be a safe integer')
  const data = value.data
  if (!isJsonRecord(data)) chunkRowMalformed(tag, 'data must be an object')
  if (tag === 'tool-call-chunks') {
    ...
    if (!withName && !hasExactKeys(data, ['turn', 'step', 'index', 'id', 'dt', 'args'])) {
      chunkRowMalformed(tag, 'data must be exactly {turn, step, index, id, name?, dt, args}')
    }
    if (typeof data.id !== 'string' || (withName && typeof data.name !== 'string')) {
      chunkRowMalformed(tag, 'id (and name when present) must be strings')
    }
    validateChunkRunData(tag, data, 'args')
  } else {
    if (!hasExactKeys(data, ['turn', 'step', 'index', 'dt', 'texts'])) {
      chunkRowMalformed(tag, 'data must be exactly {turn, step, index, dt, texts}')
    }
    validateChunkRunData(tag, data, 'texts')
  }
  return value
}

/** Expand a validated chunk row back into its exact original events, in order. */
function expandChunkRow(row) {
  ...
  for (let k = 0; k < members.length; k += 1) {
    ...
    events.push({
      ...
      seq: admitSessionSeq(row.seq0 + k),
      time,
...
/**
 * Decode one parsed JSONL storage record into its session event(s). Chunk-row
 * tagged values validate and expand; every other value passes through as a
 * single event (which also covers generation-3 records, whose chunk data rides
 * inside `assistant/message` rather than as standalone rows).
 * @param value - one line's JSON.parse result.
 * @returns the stored events, in log order.
 */
function defaultDecodeStorageRecord(value) {
  if (!isJsonRecord(value)) return [value]
  const tag = value.type
  if (tag !== 'text-chunks' && tag !== 'reasoning-chunks' && tag !== 'tool-call-chunks') {
    if (typeof value.seq === 'number') admitSessionSeq(value.seq)
    return [value]
  }
...
/**
 * Pick the session log this host reads for one session directory: the highest
 * format generation present, preferring zstd compression on ties.
 * @param sessionDir - the session's directory.
 * @returns the chosen log entry, or undefined when it holds no canonical log.
 */
function pickSessionLog(sessionDir) {
  let best // { version, compression }
  try {
    for (const entry of readdirSync(sessionDir, { withFileTypes: true })) {
      ...
      if (best === undefined || version > best.version || (version === best.version && zstd && !best.zstd)) {
        best = {
          ...
          file: join(sessionDir, entry.name),
        }
      }
    }
  } catch { /* unreadable dir: leave best undefined */ }
  return best
}

/** Every stored session log under the root (the current generation per session). */
function discoverSessions(root) {
  ...
  for (const project of projects) {
    ...
    for (const session of sessions) {
      ...
      const picked = pickSessionLog(join(root, project.name, session.name))
      if (picked === undefined) continue
      found.push({
        id: session.name,
        project: project.name,
        file: picked.file,
        name: picked.file.split('\\').pop().split('/').pop(),
        compression: picked.compression,
...
/**
 * Read a stored artifact's plaintext from disk, decompressing concatenated zstd
 * frames when compressed. Used on hosts whose persistence backend no longer
 * exposes the legacy `readRaw(id)` surface (dsh >= 0.1.5).
 * @param entry - the discovered log entry.
 * @returns `{ content }` plaintext; `readRaw` callers rely on this shape.
 */
function readRawFile(entry) {
  const bytes = readFileSync(entry.file)
  if (entry.compression === 'none') return { content: Buffer.from(bytes).toString('utf8') }
  return { content: decompressZstdFrames(bytes).toString('utf8') }
}

/**
 * The backend that owns the stored logs, normalized across host generations.
 *
 * Legacy hosts (dsh < 0.1.5) expose `readRaw(id)` / `loadStored(id)` on
 * `ctx.sessionPersistence`. The format-aware host (dsh >= 0.1.5) switched to a
 * handle-based API (`open(id, 'read')` → handle with `read()`/`close()`) and no
 * longer exposes raw text, so `readRaw` reads the stored artifact from disk and
 * `loadStored` drains the open handle. Never fails the plugin apply: if no
 * backend surface is available it reports `unavailable` and per-op code keeps
 * returning clear statuses instead of throwing at load.
 */
function backend(deps) {
  const persistence = deps.ctx.get('sessionPersistence')
  if (persistence === undefined) {
    return { generation: 'unsupported', unavailable: true, reason: 'no sessionPersistence service' }
  }
  if (typeof persistence.readRaw === 'function' && typeof persistence.loadStored === 'function') {
    // Legacy host: the backend still decodes and returns raw text itself.
    return {
      generation: 'legacy',
      readRaw: async (entry) => persistence.readRaw(entry.id),
      loadStored: async (entry) => persistence.loadStored(entry.id),
    }
  }
  if (typeof persistence.open !== 'function') {
    return { generation: 'unsupported', unavailable: true, reason: 'sessionPersistence has no readRaw/loadStored/open' }
  }
  // Format-aware host: slice the stored artifact and drain the open handle.
  return {
    generation: 'format',
    readRaw: async (entry) => readRawFile(entry),
    loadStored: async (entry) => {
      let handle
      try {
        handle = await persistence.open(entry.id, 'read')
      } catch (cause) {
        // A session that has no artifact in the backend is "missing", not an error.
...
/** Whether a normed backend error means the session artifact does not exist. */
function isNotFoundError(cause) {
  ...
  return name.includes('NotFound') || message.includes('not found') || message.includes('no such')
}
...
async function inspectSession(deps, entry) {
  ...
  try {
    const stored = await persistence.loadStored(entry)
    ...
    const errorCause = cause instanceof Error && cause.cause instanceof Error ? cause.cause : cause
    const message = errorCause instanceof Error ? errorCause.message : String(errorCause)
...
async function repairOne(deps, entry, request, helpers) {
  ...
  const raw = await persistence.readRaw(entry)
...
async function opVerify(deps, request) {
  ...
  const results = await mapLimit(targets, SCAN_CONCURRENCY, async (entry) => {
    try {
      const stored = await backend(deps).loadStored(entry)
...
async function runOp(deps, request, helpers) {
  ...
  if (op === 'status') {
    ...
    const backendName = (() => {
      if (persistence === undefined) return 'unsupported'
      if (typeof persistence.readRaw === 'function' && typeof persistence.loadStored === 'function') return 'jsonl'
      return typeof persistence.open === 'function' ? 'format' : 'unsupported'
    })()
    return {
      ok: true,
...
/**
 * Load the host packages this plugin builds on.
 *
 * `@deepseek-ai/dsh-tools` must export `defineTool` (it does on every supported
 * host). `@deepseek-ai/dsh-session` used to export `decodeStorageRecord`, but
 * dsh >= 0.1.5 removed it, so the plugin uses the host export when present and
 * otherwise falls back to the vendored, self-contained expander
 * `defaultDecodeStorageRecord`. This loader never fails the plugin apply on the
 * absence of a host symbol — compatibility is handled here, not by crashing the
 * boot.
 */
async function loadHostModules() {
  const tools = await loadHostPackage('@deepseek-ai/dsh-tools')
  if (typeof tools.defineTool !== 'function') {
    throw new Error('@deepseek-ai/dsh-tools does not export defineTool')
  }
  let decodeStorageRecord = defaultDecodeStorageRecord
  try {
    const session = await loadHostPackage('@deepseek-ai/dsh-session')
    if (typeof session.decodeStorageRecord === 'function') {
      decodeStorageRecord = session.decodeStorageRecord
    }
  } catch { /* no session package / export on this host: use the vendored decoder */ }
  return { defineTool: tools.defineTool, decodeStorageRecord }
}
